  ...
  return {
    ...
    reassemble_file: function (id) {
      ...
      try {
        ...
        // 下载完成后自动重置解码器（web 端无 reset API，等效官方"重启解码器"），
        // 使发送端队列中下一个文件的 encode_id 流能被全新初始化接收。
        if (typeof Recv !== 'undefined' && Recv.schedule_reset) {
          Recv.schedule_reset();
        }
      } catch (error) {
...
var Recv = function () {
  ...
  return {
    ...
    schedule_reset: function () {
      // 单个文件解码完成后自动重置（web 端无 reset API，等效官方"重启解码器"）。
      // 延迟足够时间让下载完成，再 reload 页面，使发送端队列中下一个文件的流能被接收。
      setTimeout(function () {
        Recv.set_HTML("errorbox", "Download complete. Preparing for next file...");
        setTimeout(function () {
          location.reload();
        }, 1500);
      }, 2500);
